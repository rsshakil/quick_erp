                        ...
                        <tbody>
                            <tr v-for="(user, i) in users" :key="i">
                                ...
                                <td>
                                    ...
                                    <router-link :to="{ name: 'user_update', query: { user_id: user.id } }" class="btn btn-info" v-can="['user_profile_view']"><b-icon icon="eye-fill" font-scale="1.2"></b-icon> {{myLang.view}}</router-link>
                                    <router-link :to="{ name: 'password_reset', query: { user_id: user.id } }" class="btn btn-warning" v-can="['change_password']"><b-icon icon="pencil-square" font-scale="1.2"></b-icon> {{myLang.change_password}}</router-link>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-12">
                                    <b-form-input v-model="email" type="email" placeholder="Email" id="email"></b-form-input>
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="phone" class="col-md-12 col-form-label">Phone</label>
                                <div class="col-md-12">
                                    <b-form-input id="phone" v-model="phone" type="text" placeholder="Phone number"></b-form-input>
                                </div>
                            </div>
...
export default {
  ...
  methods: {
    ...
    permissionForRole(options){
        ...
        // return 0;
        axios.post("get_permission_for_roles",permission_data)
        .then(({ data }) => {
            // made array unic
    ...
    save_user(){
        ...
        var user_data={name:this.name,email:this.email,password:this.password,phone:this.phone,roles:role_id,permissions:this.selected_permissions}
        axios.post("users",user_data)
        .then(({ data }) => {
        //   this.sweet_normal_alert();
    ...
    //Delete User
            ...
            permissionView(user_id,user_name){
                ...
                axios.post("permissions_by_users",user_data)
                .then(({ data }) => {
