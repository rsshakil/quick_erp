					...
					<table id="permission_list_tbl" class="table mb-0">
						...
						<tbody>
							<tr v-for="(item, i) in permissions" :key="item.id">
								...
								<td>
									<b-button variant="info" :disabled="is_disabled(item.is_system==0?true:false)" @click.prevent="editPermission(item)" v-can="['permission_update']"> <b-icon icon="pencil-square" font-scale="1.2"></b-icon></b-button>
								</td>
								<td>
									<b-button variant="danger" :disabled="is_disabled(item.is_system==0?true:false)" @click.prevent="deletePermission(item.id)" v-can="['permission_delete']"><b-icon icon="trash-fill" font-scale="1.2"></b-icon></b-button>
								</td>
							</tr>
						</tbody>
...
export default {
  ...
  methods: {
    //get Table data
    loadPermissionData() {
      axios.get("permission").then(({ data }) => {
    ...
    //Insert Permission
    SubmitPermission() {
      this.form.post("permission").then(({ data }) => {
    ...
    //Delete Permoission
            deletePermission(id){
              ...
              if (result.value) {
                ...
                this.form.delete('permission/'+id)
                    .then(({data})=> {
